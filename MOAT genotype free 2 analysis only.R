#MOAT Analysis - Prior genotype free
#clean-up
rm(list=ls())

### THINK ABOUT BACKGROUND CORRECTION ###
...
wd <- ("c:/Users/Drew/Desktop/moat/Result_Summaries_etc/Extra_Multiplex/0 markers on first run/824702 H084/combined/")
#wd <- ("X:/BespokeRES/analysis/moat/Result_Summaries_etc/Extra_Multiplex/")
setwd (wd)


###Find and select relevant files###
files <- list.files(wd, pattern="Raw_Data_com.csv", recursive=T)
show(files)
...
for (patient in 1:length(files)){
  ...
  backlength <- sum(nchar(as.character(SNPcsv[1:5,2])))+8+9+4
  resfol <- substr(files[patient], 1, nchar(files[[patient]])-backlength) #need to correct
  setwd(paste0(wd,resfol))
  
###importing and converting data files###
#SNPcsv <- read.csv("X:/BespokeRES/analysis/moat/18-5150/18-5150_ID18_18G12231_911174_MOAT_L037_cfDNA_Results/18-5150_ID18_18G12231_911174_MOAT_L037_cfDNA_Raw_Data-13-7.csv", header = FALSE) #root folder with run data
MOATpanel <- read.csv("X:/BespokeRES/scripts/resources/panel_template_MOAT.csv")
...
# MOAT ANALYSIS of cfDNA will not be able to detect fractions greater than 14% (i.e 100-86)
# if cfDNA is expected to be greater than 14% then recipient genotype from gDNA sample is required (potentially lung Tx patients only)
# would require modified script to use gDNA genotypes
##########################################################################################################################
...
for(i in 1:SNPs){
  ...
  Fail <- is.nan(Afrq) # ?not actually working, still appearing as NaN in Processed_Data
  ...
  ###*** to add option for recipient genotype for greater accuracy, might be essential for lung samples ***###
  if (Afrq > 0.86) {
    genotype <- "AA"
  } else if (Bfrq > 0.86) {
    genotype <- "BB"
...
#remove poor SNPs with count less than 6337 (less than 2SDs of average SNP count from 1st 192 samples)
poorSNPsrow <- 0
for(i in 1:SNPs){
...
#output informative SNPs (AA or BB)
#INFORMATIVE SNPS for analysis: Recipient AA or BB
AASNPs <- ""
BBSNPs <- ""

#informativeSNPsAA <- (goodSNPs[grep("AA", goodSNPs[,3]),])
#try (AASNPs <- (informativeSNPsAA[,"MAF"]))
#informativeSNPsBB <- (goodSNPs[grep("BB", goodSNPs[,3]),])
#try (BBSNPs <- (informativeSNPsBB[,"MAF"]))
#informativeSNPs <- (c(AASNPs,BBSNPs))
...
#?consider set seed
#could have used rbind rather than preformed matrix...
